...
function handleAction(notification, action, data) {
  ...
  // Getting handler from callbackFunArray by action_id
  // action_id is pointer value of 'data'
  /*if (!data.isNull()) {
    var action_id = ctypes.cast(data, ctypes.uintptr_t).value.toString();
    for (var i in callbackFunArray) {
         //console.log("handleAction",callbackFunArray[i]["action_id"],callbackFunArray[i]["handler"]);
         if (callbackFunArray[i]["action_id"] === action_id) {
             callbackFunArray[i]["handler"]();
             break;
         }
    }
  } else {
        console.log("Action data is null!");
        return;
  }*/
  ...
  if (!data.isNull()) {
    ...
    var action_id = ctypes.cast(data, ctypes.uintptr_t).value.toString();
    var i = callbackFunArray.length
    //console.log(">> Deleting actions",i);
    while (i > 0 && callbackFunArray.length > 0) {
      i--;
      //console.log("Deleting action1 >",i);
      if (callbackFunArray[i]["action_id"] === action_id) {
             callbackFunArray[i]["handler"]();
             //break;
         } else {
	   //console.log("Deleting action2");
	   //callbackFunArray.splice(i, 1);
	   //console.log("Deleting action3");
	 }
    }
  } else {
    console.log("Action notification is null!");
    return;
  }

  // Deleting actions for this specific ID
  /*if (!notification.isNull()) {
    var notification_id = action.readString().split("_")[1];
    var i = callbackFunArray.length
    console.log(">> Deleting actions",i);
    while (i > 0 && callbackFunArray.length) {
      i--;
      console.log("Deleting action1 >",i);
      if (callbackFunArray[i]["notification_id"] === notification_id) {
	console.log("Deleting action2");
	callbackFunArray.splice(i, 1);
	console.log("Deleting action3");
      }
  ...
  // Clearing callbackFunArray
  //console.log("Deleting callbackFunArray1 length=",callbackFunArray.length);
  //callbackFunArray.length = 0;
  //console.log("Deleting callbackFunArray2 length=",callbackFunArray.length);
...
linux.notifyWithActions = function(iconURL, title, text, notifier, closeHandler, actionsList) {
    ...
    // Escaping all html markups
    // TODO : Some markups are allowed and should not be escaped
    ...
    if (actionsList) {
        for (var i in actionsList) {
            ...
            if (handler && typeof(handler) === "function") {
                ...
                callbackFunArray.push({
                    "action_id": action_id,
                    "notification_id": notification_id,
                    "handler": handler
                });
                //console.log("notifyWithActions",i,notification_id,action_id,handler);
                notify_notification_add_action(notification, ctypes.char.array()("gnotifier_"+notification_id+"_" + i), ctypes.char.array()(label), actionFunType(handleAction), user_data_ptr, null);
            }
        }
    }
    ...
    if (closeHandler && typeof(closeHandler) === "function") {
        ...
        g_signal_connect_data(notification, ctypes.char.array()("closed"), callbackFun, notification, null, 0);
    }
