...
thunderbird.mailListener = {
  ...
  OnItemIntPropertyChanged: function (aItem,aProperty,aOldValue,aNewValue) {
    ...
    function getFirstFolderWithNewMail(aFolder) {
      if (aFolder) {
	//console.log("aFolder.URI: "+aFolder.URI);
	//console.log("aFolder.biffState: "+aFolder.biffState);
	//console.log("aFolder.hasNewMessages: "+aFolder.hasNewMessages);
    ...
    if (aProperty=="BiffState" && aNewValue==0) {
      ...
      // If folder found, getting new message
      if (folder && !sps['simpleNewMail']) {
	var msg = folder.firstNewMessage;
	var text = msg.mime2DecodedAuthor + "\n" + msg.mime2DecodedSubject;
      } else {
	
	// New message not found, doing fallback text
	// or simple notifications enabled (simpleNewMail == true)
