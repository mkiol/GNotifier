...
* changelog:
* [1.8.0 beta]
* - Clickable download complete notification
* [1.7.9]
...
Task.spawn(function () {
  try {
	...
	if(sps['downloadCompleteAlert'] && download.succeeded) {
	  ...
	  var body = "<a href='file:///"+dir+"'>"+_("download_finished")+"<a>";
	  //notifyNative(icon, _("download_finished"), filename);
	  console.log(body);
	  notifyNative(icon,body,filename);
	}
      }
    };
...
AlertsService.prototype = {
  ...
  showAlertNotification: function GNotifier_AlertsService_showAlertNotification(
    ...
    //console.log("GNotifier_AlertsService_showAlertNotification");
...
function notifyNative(iconURL, title, text, notifier) {
  ...
  //console.log("notifyNative: title = " + title);
  
  var ret = notifApi.notify(iconURL, title, text, system.name);
  if(!ret) {
...
exports.main = function(options, callbacks) {
  ...
  if(notifApi.checkAvailable()) {
    ...
    if (sps['replaceAlerts'] && system.platform != "darwin") {
      ...
      //console.log("replaceAlerts is ok");
